...
/*   Updated: 2024/02/20 19:23:50 by olimarti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_vector4d *coord)
{
	...
	if ((context->visited[(int)below_current.y][(int)below_current.x]
	...
	t_vector4d *coord)
{
	...
	if ((context->visited[(int)left_of_current.y][(int)left_of_current.x]
		& (RIGHT | DOWN)) == (RIGHT | DOWN) &&
	...
	if ((context->visited[(int)left_of_current.y][(int)left_of_current.x]
		...
		(context->visited[(int)right_of_current.y][(int)right_of_current.x]
			& RIGHT))
		return (true);
	return (false);
}

bool	check_diagonal_right(
	t_edge_exploration_context *context,
	t_vector4d *coord)
{
	const t_point2d	next_point = (t_point2d){{coord->x + 1, coord->y + 1}};
	...
	const t_point2d	right_of_next
		= (t_point2d){{next_point.x + 1, next_point.y}};

	if ((context->visited[(int)right_of_current.y][(int)right_of_current.x]
		& (RIGHT | DOWN)) == (RIGHT | DOWN) &&
	...
	if ((context->visited[(int)below_current.y][(int)below_current.x]
		& (RIGHT | DOWN)) == (RIGHT | DOWN) &&
		(context->visited[(int)right_of_next.y][(int)right_of_next.x]
			& RIGHT) &&
		(context->visited[(int)next_point.y][(int)next_point.x] & RIGHT))
		return (true);
	return (false);
